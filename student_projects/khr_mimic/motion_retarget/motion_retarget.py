...
def build_markers(num_markers, col=[0.0, 0.0, 0.0, 1.0], size=0.01):
    marker_radius = size
    markers_handle = []
...
def get_joint_limits2(robot):
    ...
    for i in range(num_joints):
        ...
        if (
            joint_type == pybullet.JOINT_PRISMATIC
            or joint_type == pybullet.JOINT_REVOLUTE
            or joint_type == pybullet.JOINT_FIXED
        ):
            joint_lower_bound.append(joint_info[8])
            joint_upper_bound.append(joint_info[9])
            joint_limit_range.append(joint_info[9] - joint_info[8])
...
def set_pose(robot, pose):
    ...
    pybullet.resetBasePositionAndOrientation(robot, root_position, root_quaternion)
        ...
        for i in range(num_pos):
            ...
            curr_pos = pose3d.QuaternionRotatePoint(
                ...
                transformations.quaternion_from_euler(
                    0.5 * np.pi, 0, 0.5 * np.pi, axes="sxyz"
                ),
            )
...
def retarget_root_pose(ref_joint_pos):
    ...
    y_direction = ref_joint_pos[left_upleg_index] - ref_joint_pos[right_upleg_index]
    ...
    rotation_matrix = np.array(
        [
            [x_direction[0], y_direction[0], z_direction[0], 0],
...
def scale_ref_pos(robot, ref_joint_pos):
    ...
    joint_lower_bound, joint_upper_bound, joint_limit_range = get_joint_limits(robot)
    ...
    ref_shoulder1_pos = ref_joint_pos[
        [
            bvh_cfg.BVH_JOINT_NAMES.index("LeftShoulder"),
            bvh_cfg.BVH_JOINT_NAMES.index("RightShoulder"),
        ]
    ]
    ref_shoulder2_pos = ref_joint_pos[
        [
            bvh_cfg.BVH_JOINT_NAMES.index("LeftArm"),
            bvh_cfg.BVH_JOINT_NAMES.index("RightArm"),
        ]
    ]
    ref_elbow_pos = ref_joint_pos[
        [
            bvh_cfg.BVH_JOINT_NAMES.index("LeftForeArm"),
            bvh_cfg.BVH_JOINT_NAMES.index("RightForeArm"),
        ]
    ]
    ref_hand_pos = ref_joint_pos[
        [
            bvh_cfg.BVH_JOINT_NAMES.index("LeftHand"),
            bvh_cfg.BVH_JOINT_NAMES.index("RightHand"),
        ]
    ]
    ref_finger1_pos = ref_joint_pos[
        [
            bvh_cfg.BVH_JOINT_NAMES.index("LeftFingerBase"),
            bvh_cfg.BVH_JOINT_NAMES.index("RightFingerBase"),
        ]
    ]
    ref_finger2_pos = ref_joint_pos[
        [
            bvh_cfg.BVH_JOINT_NAMES.index("LeftHandIndex1"),
            bvh_cfg.BVH_JOINT_NAMES.index("RightHandIndex1"),
        ]
    ]
    ref_thumb_pos = ref_joint_pos[
        [
            bvh_cfg.BVH_JOINT_NAMES.index("LThumb"),
            bvh_cfg.BVH_JOINT_NAMES.index("RThumb"),
        ]
    ]

    ref_hips_pos = ref_joint_pos[
        [
            bvh_cfg.BVH_JOINT_NAMES.index("LHipJoint"),
            bvh_cfg.BVH_JOINT_NAMES.index("RHipJoint"),
        ]
    ]
    ref_crotch_pos = ref_joint_pos[
        [
            bvh_cfg.BVH_JOINT_NAMES.index("LeftUpLeg"),
            bvh_cfg.BVH_JOINT_NAMES.index("RightUpLeg"),
        ]
    ]
    ref_knee_pos = ref_joint_pos[
        [
            bvh_cfg.BVH_JOINT_NAMES.index("LeftLeg"),
            bvh_cfg.BVH_JOINT_NAMES.index("RightLeg"),
        ]
    ]
    ref_foot_pos = ref_joint_pos[
        [
            bvh_cfg.BVH_JOINT_NAMES.index("LeftFoot"),
            bvh_cfg.BVH_JOINT_NAMES.index("RightFoot"),
        ]
    ]
    ref_toe_pos = ref_joint_pos[
        [
            bvh_cfg.BVH_JOINT_NAMES.index("LeftToeBase"),
            bvh_cfg.BVH_JOINT_NAMES.index("RightToeBase"),
        ]
    ]
    ...
    # scale upper body
    scaled_hip_to_spine_pos_delta = (
        ref_hip_to_spine_pos_delta * khr_cfg.HIP_TO_SPINE_SCALE
    )
    scaled_spine_to_torso_pos_delta = (
        ref_spine_to_torso_pos_delta * khr_cfg.SPINE_TO_TORSO_SCALE
    )
    scaled_torso_to_neck_pos_delta = (
        ref_torso_to_neck_pos_delta * khr_cfg.TORSO_TO_NECK_SCALE
    )
    scaled_neck_to_neck1_pos_delta = (
        ref_neck_to_neck1_pos_delta * khr_cfg.NECK_TO_NECK1_SCALE
    )
    scaled_neck1_to_head_pos_delta = (
        ref_neck1_to_head_pos_delta * khr_cfg.NECK1_TO_HEAD_SCALED
    )

    scaled_joint_pos[bvh_cfg.BVH_JOINT_NAMES.index("Spine")] = (
        ref_hip_pos + scaled_hip_to_spine_pos_delta
    )
    scaled_joint_pos[bvh_cfg.BVH_JOINT_NAMES.index("LowerBack")] = scaled_joint_pos[
        bvh_cfg.BVH_JOINT_NAMES.index("Spine")
    ]
    scaled_joint_pos[bvh_cfg.BVH_JOINT_NAMES.index("Spine1")] = (
        scaled_joint_pos[bvh_cfg.BVH_JOINT_NAMES.index("Spine")]
        + scaled_spine_to_torso_pos_delta
    )
    scaled_joint_pos[bvh_cfg.BVH_JOINT_NAMES.index("Neck")] = (
        scaled_joint_pos[bvh_cfg.BVH_JOINT_NAMES.index("Spine1")]
        + scaled_torso_to_neck_pos_delta
    )
    scaled_joint_pos[bvh_cfg.BVH_JOINT_NAMES.index("Neck1")] = (
        scaled_joint_pos[bvh_cfg.BVH_JOINT_NAMES.index("Neck")]
        + scaled_neck_to_neck1_pos_delta
    )
    scaled_joint_pos[bvh_cfg.BVH_JOINT_NAMES.index("Head")] = (
        scaled_joint_pos[bvh_cfg.BVH_JOINT_NAMES.index("Neck1")]
        + scaled_neck1_to_head_pos_delta
    )

    scaled_torso_to_shoulder1_pos_delta = (
        ref_torso_to_shoulder1_pos_delta * khr_cfg.TORSO_TO_SHOULDER1_SCALE
    )
    scaled_shoulder1_to_shoulder2_pos_delta = (
        ref_shoulder1_to_shoulder2_pos_delta * khr_cfg.SHOULDER1_TO_SHOULDER2_SCALE
    )
    scaled_shoulder2_to_elbow_pos_delta = (
        ref_shoulder2_to_elbow_pos_delta * khr_cfg.SHOULDER2_TO_ELBOW_SCALE
    )
    scaled_elbow_to_hand_pos_delta = (
        ref_elbow_to_hand_pos_delta * khr_cfg.ELBOW_TO_HAND_SCALE
    )
    scaled_hand_to_finger1_pos_delta = (
        ref_hand_to_finger1_pos_delta * khr_cfg.HAND_TO_FINGER1_SCALE
    )
    scaled_finger1_to_finger2_pos_delta = (
        ref_finger1_to_finger2_pos_delta * khr_cfg.FINGER1_TO_FINGER2_SCALE
    )
    scaled_finger2_to_thumb_pos_delta = (
        ref_finger2_to_thumb_pos_delta * khr_cfg.FIGNER2_TO_THUMB_SCALE
    )
    ...
    scaled_hips_to_crotch_pos_delta = (
        ref_hips_to_crotch_pos_delta * khr_cfg.HIPS_TO_CROTCH_SCALE
    )
    scaled_crotch_to_knee_pos_delta = (
        ref_crotch_to_knee_pos_delta * khr_cfg.CROTCH_TO_KNEE_SCALE
    )
    scaled_knee_to_foot_pos_delta = (
        ref_knee_to_foot_pos_delta * khr_cfg.KNEE_TO_FOOT_SCALE
    )
    ...
    scaled_joint_pos[
        ...
        ref_hips_pos + scaled_hips_to_crotch_pos_delta
    )
    scaled_joint_pos[
        [
            bvh_cfg.BVH_JOINT_NAMES.index("LeftLeg"),
...
def retarget_pose(robot, ref_joint_pos):
    ...
    non_fixed_joint_indices = get_non_fixed_joint_indices(robot)
    joint_lower_bound, joint_upper_bound, joint_limit_range = get_joint_limits(robot)
    joint_lower_limit, joint_upper_limit, joint_limit_range = get_joint_limits2(robot)
    ...
    target_joint_indices = (
        target_elbow_indices + target_hand_indices + target_foot_indices
    )
    ...
    target_robot_joint_indices = [
        ...
        robot_joint_indices["LARM_JOINT2"],
        robot_joint_indices["RARM_JOINT2"],
        robot_joint_indices["LHAND_VIRTUAL_JOINT"],
        robot_joint_indices["RHAND_VIRTUAL_JOINT"],
        # leg target
        robot_joint_indices["LLEG_VIRTUAL_JOINT2"],
        robot_joint_indices["RLEG_VIRTUAL_JOINT2"],
        robot_joint_indices["LLEG_VIRTUAL_JOINT3"],
        robot_joint_indices["RLEG_VIRTUAL_JOINT3"],
    ]
    ...
    DEFAULT_JOINT_POS = np.array(joint_lower_limit) + np.array(joint_upper_limit) / 2.0

    target_joint_pose = pybullet.calculateInverseKinematics2(
        robot,
...
def retarget_motion(robot, ref_joint_pos):
    ...
    for f in range(num_frames):
        ...
        curr_pose, scaled_joint_pos = retarget_pose(robot, ref_joint_position)
...
def set_robot_joint_marker(robot, marker_ids):
    ...
    robot_joint_pos = np.array(
        pybullet.getLinkStates(
            robot, list(range(num_joints)), computeForwardKinematics=True
        )
    )[:, 4]
...
def build_world():
    ...
    # create ground
    ground = pybullet.loadURDF("plane_implicit.urdf", basePosition=[0.0, 0.0, 0.0])

    # create actor
    robot = pybullet.loadURDF(
        khr_cfg.ROBOT_URDF_FILENAME,
...
def main():
    ...
    robot_joint_marker_ids = build_markers(
        num_joints, col=[0.0, 0.0, 1.0, 1.0], size=0.005
    )
    scaled_bvh_joint_pos_marker_ids = build_markers(
        num_bvh_joints, col=[0.9, 0.0, 0.7, 1.0], size=0.01
    )
    ...
    for i in range(len(bvh_file_list)):
        ...
        for frame in range(num_frames):
            ref_joint_pos.append(pre_process_ref_joint_pos(ref_joint_pos_frames[frame]))
