...
class KHRMimicEnv(MujocoEnv, utils.EzPickle):
    ...
    def set_param(self):
        ...
        print("jnt_pos_indices", self.jnt_pos_indices)
        print("jnt_vel_indices", self.jnt_vel_indices)
    ...
    def step(self, action):  # action : joint torque (17)
        ...
        """
        current_frame = int(
            (self.dt / self.frame_duration) * self.step_cnt % self.motion_cycle_frames
        )
        # current_frame = self.step_cnt % self.motion_cycle_frames
        self.kp = 10.0
        # target_jnt_pos = np.clip(action * np.pi, -np.pi, np.pi)
        # target_jnt_pos[-1] = 0.0  # head joint is not used
        target_jnt_pos = np.zeros(17, dtype=np.float32)
        target_jnt_pos[:-1] = self.ref_jnt_pos[current_frame, :]
        # target_jnt_pos[10] = self.ref_jnt_pos[current_frame, 10]
    ...
    def reset_model(self):
        ...
        # qpos[7:] = 0.0
        qvel[:] = 0.0
        qpos[:3] = self.ref_base_pos[0, :3]
        qpos[3:7] = self.ref_base_quat[0, :]
        qpos[7:-1] = self.ref_jnt_pos[0, :]
        qpos[-1] = 0.0
