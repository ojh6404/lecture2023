...
class KHRMimicRunEnv(MujocoEnv, utils.EzPickle):
    def __init__(
        self,
        test=False,
        max_step=20000000,
        ...
        motion_data_path = os.path.join(root_dir, "data", "blended_processed_09_01.npz")
    ...
    def step(self, action):  # action : joint torque (17)
        ...
        if (self.frame_cnt // self.motion_cycle_frames) and (
            (self.sim.data.qpos[0] - self.ref_base_pos[self.init_motion_data_frame, 0])
            < 0.5 * (self.frame_cnt // self.motion_cycle_frames)
        ):
            not_forward = True
            # TODO
            reward -= 10.0
    ...
    def render(self, *args, **kwargs):
        ...
        super(KHRMimicRunEnv, self).render(*args, **kwargs)
